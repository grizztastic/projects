...
class CWL2Attacker(Attacker):
    ...
    def _define_ops(self):
        ...
        # >>> Your code here <<<
        '''Most of the code was created through the direction of the PGD attacker class given in hw4_part1.py'''
        self.W = tf.Variable(tf.zeros(self.batch_shape), name="W")  # initialize a W tensor
        self.init_inputs.append(tf.assign(self.W, (tf.math.atanh(self.X_var * 2 - 1)))) #set W to arctan (2x - 1_

        # perturbation itself
        self.perturbation = (tf.tanh(self.W) + 1) * 0.5 - self.X_var  # calculation for delta as per the paper
        self.adv_image = (tf.tanh(self.W) + 1) * 0.5  # calculation for an adversarial image as per the paper
        logits = self.model.logits(self.adv_image)  # get logits output from model with adversarial image as input

        correct_class = tf.reduce_sum(logits * self.Yi, axis=1)  # generate correct class values
        other_classes = tf.reduce_max(logits * (1 - self.Yi), axis=1)  #generate other class values non targetted
        other_classes_targetted = (logits * (1 - self.Yi))[:, self.target] #generate other class values targetted
        l2dist = tf.reduce_sum(tf.square(self.perturbation), [1, 2, 3])  # minimize perturbation (l2 distance calculation)

        if self.target is None:  # untargetted attack
            self.loss = tf.maximum(-self.k, correct_class - other_classes) * self.c + l2dist
        else:  # targetted attack
            self.loss = tf.maximum(-self.k, correct_class - other_classes_targetted) * self.c + l2dist

        grad = tf.gradients(ys=self.loss, xs=self.W)[0]  # get gradients of loss w.r.t W
        W_updated = tf.assign(self.W, self.W - self.lr * grad)  # update value of W

        self.step = [W_updated, tf.assign(self.lr, self.lr * self.learning_rate_decay)]  # To be executed each step.

        self.output = tf.clip_by_value(self.adv_image, 0, 1)
        # >>> End of your code <<<
